...
module.exports = {
    ...
    run: async (client, message) => {
        const pc = await db.consu.findOne({consumidor: !message.author ? message.user.id:message.author.id})
        const lan = await db.lgs.findOne({guildID: message.guild.id})
        if (!lan) {
            if (message.mentions.users.first() || client.users.fetch(!args[0] ? message.options.getString('vitima').replace(/[<@!>]/g, ''):args[0])) {
                if (!lan) {
                    if (!pc) return message.reply('Você nao esta cadastrado na loja')
                    const money = await db.coins.findOne({id: !message.author ? message.user.id:message.author.id})
                    ...
                    if (pc.cooldown + 3.6e+6 > Date.now()) {
                        ...
                        await db.consu.findOneAndUpdate({consumidor: !message.author ? message.user.id:message.author.id}, {cooldown: Date.now()})
                        await db.consu.findOneAndUpdate({consumidor: !message.author ? message.user.id:message.author.id}, {$pull: {produtos: 'computador'}})
                        ...
                        message.reply(message.author, embed).then(editar => {
                            ...
                            setTimeout(async () => {
                                ...
                                await db.coins.findOneAndUpdate({id: !message.author ? message.user.id:message.author.id}, {coinsc: money.coinsc + grana})
                    ...
                    if (lan.lang === 'en') {
                        ...
                        if (pc.cooldown + timeout > Date.now()) {
                            ...
                            await db.consu.findOneAndUpdate({consumidor: !message.author ? message.user.id:message.author.id}, {cooldown: Date.now()})
                            await db.consu.findOneAndUpdate({consumidor: !message.author ? message.user.id:message.author.id}, {$pull: {produtos: 'computer'}})
                            const grana = Math.floor(Math.random() * 600);
                            const embed = new MessageEmbed()
                                .setColor('#9900f8')
