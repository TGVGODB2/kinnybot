...
module.exports = {
    ...
    run: async (client, message, args) => {
        const vit = message.mentions?.users.first() || client.users.fetch(!args[0] ? message.options.getString('user').replace(/[<@!>]/g, ''):args[0])
        ...
        if (autor.robdown + timeout > Date.now()) {
            ...
            if (vitima) {
                ...
                if (vitima.id === !message.author ? message.user.id:message.author.id) return message.reply(`${client.user.username} - Erro \n Que bandido tu é? Pra roubar a si mesmo? Eu te levo pro hospicio heim.`)
                if (['395995293436477442'].includes(vit.id)) {
                 await db.coins.updateOne({id: !message.author ? message.user.id:message.author.id}, {robdown: Date.now()})
                    const embed = new MessageEmbed()
                        .setColor('#9900f8')
