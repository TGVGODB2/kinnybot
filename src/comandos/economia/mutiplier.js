...
module.exports = {
    ...
    run: async(client, message) => {
        ...
        let premi = await db.premi.findOne({groupid: message.author.id})
        ...
        let member = await db.coins.findOne({id: message.author.id})
        ...
        if (member.multidown + timeout > Date.now()) {
            ...
            await db.coins.findOneAndUpdate({id: message.author.id}, {multidown: Date.now()})
            const embed = new MessageEmbed()
                .setColor('#9900f8')
                .addField(`${client.user.username} - Diversão`, `Seus koins foi multiplicado por ${grana}!`)
            await db.coins.findOneAndUpdate({id: message.author.id}, {coinsc: member.coinsc * grana, multidown: Date.now()})
            message.channel.send(embed)
        }
    }
}