...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(mencao.id === !message.author ? message.user.id:message.author.id) return message.reply(`${client.user.username} - Erro \n Você não pode pagar a si mesmo!`)
        const autor1 = await db.coins.findOne({id: mencao.id})
        const autor2 = await db.coins.findOne({id: !message.author ? message.user.id:message.author.id})
        if(!autor1 || !autor2) return message.reply('Umas das pessoas não tem conta!')
        const quantia = Number(args[0]) || message.options.getNumber('quantia')
