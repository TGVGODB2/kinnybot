...
module.exports = {
    ...
    run: async(client, message, args) => {
        let lang = await db.lgs.findOne({guildID: !message.author ? message.user.id:!message.author ? message.user.id:message.author.id})
        let list = ['en']
        let lang2 = args?.join(" ").toLowerCase() || message.options.getString('lang')
        ...
        if(!lang) {
            await db.lgs.create({guildID: !message.author ? message.user.id:!message.author ? message.user.id:message.author.id, lang: lang2})
            ...
            await db.lgs.findOneAndRemove({guildID: !message.author ? message.user.id:!message.author ? message.user.id:message.author.id})
