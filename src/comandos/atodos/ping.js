...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        message.reply({embeds: [embed2]}).then(edita => {
            const embed = new MessageEmbed()
                ...
                .addField(`<:mongodbkin:801203593658105898> ${lan && lan.lang === 'en' ? "Database":"Banco de dados"}`, `${Math.floor(((stop[0] * 1e9) + stop[1]) / 1e6)} ms`)
            setTimeout(() => {
