...
module.exports = {
    ...
    run: async (client, message, args) => {
        let perguntaa = args?.join(' ') || message.options.getString('responde')
        const questionEnd = perguntaa.indexOf('?')+1
        ...
        await db.res.findOneAndUpdate({pergunta: `${pergunta.replace('?', '')}?`}, {$push: {respostas: `**${resposta} - ${message.author.username}**`}})
        ...
        client.users.cache.get(res.autor).send(`Atenção! O ${message.author.username} respondeu sua pergunta! Veja!`)
