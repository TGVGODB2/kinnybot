...
module.exports = {
    ...
    run: async(client, message, args) => {
            ...
            let dispositivo = Object?.keys(membro?.presence.clientStatus)[0] || 'No platform '
            let statuse = membro.presence.status
            let customs = `${membro.presence.activities[0]?.emoji?.name || "No emoji"} ${membro.presence.activities[0]?.state || 'Sem status'}`
            const values = {web: '🌐', desktop: '🖥', mobile: '📱 '}
            const result = dispositivo.replace(/desktop|mobile|web/gi, v => values[v])
            const values2 = {
                online: '<:online:799747794629689356> Online',
                idle: '<:ausente:799747794545279057> Absent',
                offline: '<:offline:799747795237732412> Offline ',
                dnd: '<:ocupado:799747794616713286> Occupied',
                stream: '<:stremando:799747794537414686> Broadcasting'
            }
            const result2 = statuse.replace(/online|idle|dnd|offline|stream/gi, v => values2[v])
