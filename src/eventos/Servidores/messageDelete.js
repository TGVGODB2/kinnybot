...
if(message.author.bot || message.author.id === client.user.id) return;
            ...
            if (!saifora) {
                ...
                message.guild.channels.cache.get(message.channel.id).createWebhook(message.author.username, {
                    ...
                    const webhookClient = new WebhookClient(saifora.idweb, saifora.token);
                                        webhookClient.send(`\`${message?.content.replace(/`/g, '').replace(`<@!${mencao.id}>`, mencao.user.username).replace(`<@${mencao.id}>`, mencao.user.username)}\`` || mencao.user.username, {
    ...
    if(!message.author ? message.user.id:message.author.id === client.user.id) return;
